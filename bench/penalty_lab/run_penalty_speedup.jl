# μ_P speed-up experiments over a CUTEst regime, FP64. Two families, 5 configs total:
#
#   (1) s-gate ONLY (no E-gate, no μ_B coupling): bump μ_P (linear κ_P=2, muP0=2) whenever
#       ‖s_E‖∞ ≤ τ_P/μ_next.   τ_P ∈ {1, 10, 700}.
#   (2) bcoupled (bump on μ_B drop, no E-gate) but MORE AGGRESSIVE:
#       2a: muP0=10, linear κ_P=10 ;   2b: muP0=10, superlinear θ_P=1.5 (κ_P=2).
#
# Condensed KKT, default solver+scaling, tol=1e-8, acceptable off (acc_tol=tol). One shared worker
# pool, configs run sequentially.  Usage:
#   julia --project=bench/penalty bench/penalty_lab/run_penalty_speedup.jl [regime] [nworkers]
# Writes results/penalty_speedup/<regime>__<config>.csv + summary_speedup.csv.
...
const CONFIGS = [
    (name="sgate_tP1",    kind=:sgate, tau_P=1.0,   kappa_P=2.0,  theta_P=1.0, muP0=2.0),
    (name="sgate_tP10",   kind=:sgate, tau_P=10.0,  kappa_P=2.0,  theta_P=1.0, muP0=2.0),
    (name="sgate_tP700",  kind=:sgate, tau_P=700.0, kappa_P=2.0,  theta_P=1.0, muP0=2.0),
    (name="bcoup_k10",    kind=:bcoup, tau_P=0.0,   kappa_P=10.0, theta_P=1.0, muP0=10.0),
    (name="bcoup_th1.5",  kind=:bcoup, tau_P=0.0,   kappa_P=2.0,  theta_P=1.5, muP0=10.0),
]

probs = String.(filter(!isempty, strip.(readlines(joinpath(HERE, "regimes", "$REGIME.txt")))))
println("SPEEDUP sweep: regime=$REGIME problems=$(length(probs)) workers=$NW configs=$(length(CONFIGS)) (FP64)")
...
@everywhere begin
    ...
    # ---- (1) s-gate-only schedule: bump μ_P iff ‖s_E‖∞ ≤ τ_P/μ_next. No E-gate. ----
    ...
    function MadNLP.update_penalty!(s::SGateOnly, eh::KernelPenaltyEquality,
        ...
        if μ < s.muP_max
            ...
            if sE <= T(s.tau_P) / μ_next        # s-gate ONLY
    ...
    # ---- (2) barrier-coupled schedule: bump μ_P iff μ_B dropped. No E-gate, no s-gate. ----
...
outdir = joinpath(HERE, "results", "penalty_speedup"); mkpath(outdir)
...
for (ci,c) in enumerate(CONFIGS)
    ...
    open(joinpath(outdir, "summary_speedup.csv"), "w") do io
...
println("\n(ref: baseline tight=116 ; bcoupled κ2 = 53 tight/83 succ ; linear-gated tE10_k2 = 34/66)")
println("PENALTY_SPEEDUP_DONE")
