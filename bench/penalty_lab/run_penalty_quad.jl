# QUADRATIC penalty kernel sweep, FP64. φ=(μ/2)s², φ'=μs, φ''=μ (constant Hessian, NO overflow).
# Runs the three reference μ_P configs used for the cosh/normcosh comparisons:
#   bcoup_k2   : bcoupled (bump μ_P on μ_B drop), muP0=2,  κ_P=2  (the cosh champion config, =53 tight)
#   bcoup_k10  : bcoupled, muP0=10, κ_P=10
#   sgate_tP10 : s-gate only (bump iff ‖s_E‖∞ ≤ τ_P/μ_next), τ_P=10, κ_P=2, muP0=2
# Condensed KKT, default solver+scaling, tol=1e-8, muP_max=1e7. No step cap needed (quadratic never
# overflows). Usage: julia --project=bench/penalty bench/penalty_lab/run_penalty_quad.jl [regime] [nworkers]
# Writes results/penalty_quad/<regime>__<config>.csv + summary_quad.csv.
...
const CONFIGS = [
    (name="bcoup_k2",   kind=:bcoup, tau_P=0.0,  kappa_P=2.0,  theta_P=1.0, muP0=2.0),
...
println("QUAD-KERNEL sweep: regime=$REGIME problems=$(length(probs)) workers=$NW configs=$(length(CONFIGS)) (FP64, QuadraticKernel)")
...
outdir = joinpath(HERE, "results", "penalty_quad"); mkpath(outdir)
...
for (ci,c) in enumerate(CONFIGS)
    ...
    open(joinpath(outdir, "summary_quad.csv"), "w") do io
...
println("\n(cosh/normcosh refs @ muP_max=1e7: bcoupled-κ2 cosh=53 ; bcoup_k10 normcosh=44 ; sgate_tP10 normcosh=41 ; baseline 116)")
println("PENALTY_QUAD_DONE")
